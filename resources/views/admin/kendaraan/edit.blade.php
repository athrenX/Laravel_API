    ...
    <form action="{{ route('admin.kendaraan.update', $kendaraan->id) }}" method="POST" enctype="multipart/form-data">
        ...
        <select name="destinasi_id" id="destinasi_id">
            @foreach($destinasis as $destinasi)
                <option value="{{ $destinasi->id }}" {{ ($kendaraan->destinasi_id == $destinasi->id || old('destinasi_id') == $destinasi->id) ? 'selected' : '' }}>
        ...
        <input type="text" name="jenis" value="{{ old('jenis', $kendaraan->jenis) }}">
        ...
        @if($kendaraan->gambar)
            <p>Gambar saat ini: <img src="{{ asset('storage/' . $kendaraan->gambar) }}" alt="gambar" width="80"></p>
        @endif
